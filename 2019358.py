class LR1Parser:
    ...
    def closure(self, item_set):
        # Initialize closure to the given item set
        closure = item_set[:]
        
        # Iterate over each item in the closure
        i = 0
        while i < len(closure):
            item = closure[i]
            dot_pos = item[1].index(".")
            
            # If the dot is at the end of the item, move on to the next item
            if dot_pos == len(item[1]) - 1:
                i += 1
                continue
            
            # Otherwise, get the next symbol after the dot
            ...
            if next_symbol in self.non_terminals:
                for production in self.grammar:
                    if production[0] == next_symbol:
                        # Add the lookaheads from the current item
                        lookaheads = set(item[2])
                        # Add any additional terminals from the production
                        ...
                        for lookahead in lookaheads:
                            ...
                            # If the new item isn't already in the closure, add it
    ...
    def goto(self, item_set, symbol):
        ...
        # Create an empty list to hold the items in the next set
        new_item_set = []
        
        # Iterate over each item in the current set
        for item in item_set:
            dot_pos = item[1].index(".")
            
            # If the dot is before the end of the item and the next symbol is the given symbol,
            # create a new item with the dot shifted to the right of the symbol
        ...
        # If there are no items in the next set, return None
        if len(new_item_set) == 0:
            return None
        # Otherwise, compute the closure of the new item set and return it
        else:
...
def main():
    ...
    # Initialize an LR(1) parser with the grammar
    lr1_parser = LR1Parser(grammar)

    # Print the sets of LR(1) items
    ...
    # Print the LR(1) parsing table
    print("\nLR(1) parsing table:")
    print("Action:")
    for key, value in lr1_parser.action.items():
        print("({}, {}) -> {}".format(key[0], key[1], ", ".join(value)))
    print("\nGoto:")
    for key, value in lr1_parser.goto(lr1_parser.action.keys(), lr1_parser.action.values()).items():
        print("({}, {}) -> {}".format(key[0], key[1], value))

    # Test the LR(1) parser with an input string
    input_str = "a × b – c"
